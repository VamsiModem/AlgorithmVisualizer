...
class ElementContainer extends React.Component {
  ...
  merge = (temp, arr, left, mid, right, state) => {
    ...
    while (mid <= right) temp[index++] = arr[mid++];
    for (let i = 0; i < size; i++) arr[right] = temp[right--];
  };
  ...
  getMaxIndex = () => {
    if (this.state.compared.length > 0) {
      ...
      return this.state.array[index1] > this.state.array[index2]
        ? index1
        : index2;
    }
    return -1;
  ...
  render() {
    ...
    const boundaries =
      this.state.boundaries == undefined ? [] : this.state.boundaries;
    return (
              ...
              return (
                <Element
                  ...
                  width={
                    ...
                    (this.state.array.length * 1.95) /
                    10
                  }
                  total={this.state.array.length}
                  color={
                    this.props.multicolor ? this.state.colors[ki] : '#f36698'
                  }
                  ...
                  boundaries={
                    boundaries.length > 0
                      ? ki >= boundaries[0] && ki <= boundaries[2]
                      : false
                  }
                />
              );
