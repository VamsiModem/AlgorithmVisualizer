...
class NumberToString extends React.Component {
  ...
  two = (number) => {
    if (number === 0) return '';
    else if (number < 10) return this.toOne(number);
    else if (number < 20) return this.toTwoLessThan20(number);
    else {
  ...
  three = (number) => {
    ...
    } else if (hundered === 0 && remaining !== 0) res = this.two(remaining);
    else if (hundered !== 0 && remaining === 0) res = this.three(remaining);
  ...
  convertToString = () => {
    ...
    const thousand = Math.floor(
      (number - (billion * 100000000 - million * 1000000)) / 1000
    );
    const remaining =
      number - billion * 100000000 - million * 1000000 - thousand * 1000;
    let res = '';
    if (billion !== 0) res += this.three(billion) + '  Billion ';
    if (million !== 0) {
      if (res.length !== 0) {
        res += ' ';
      }
      ...
      <div style={{ width: '300px' }}>
        <div>
          <TextField
            ...
            helperText={
              isNaN(Number(this.state.value)) ? 'length cannot be > 8' : ''
            }
