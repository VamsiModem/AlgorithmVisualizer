...
class BSTComponent extends React.Component {
  constructor(props) {
    ...
    while (cur != null || stack.length > 0) {
      ...
      let parentId =
        cur.parent == null
          ? null
          : `node-${cur.parent.value}-depth-${cur.parent.y}`;
  ...
  drawPath(path, startX, startY, endX, endY) {
    ...
    const delta =
      deltaY < this.absolute(deltaX) ? deltaY : this.absolute(deltaX);
    let arc1 = 0;
    let arc2 = 1;
    if (startX > endX) {
      arc1 = 1;
  ...
  connectElements(path, startElem, endElem) {
    ...
    const svgCoords = ReactDOM.findDOMNode(
      svgContainer
    ).getBoundingClientRect();
    const startElementCoords = ReactDOM.findDOMNode(
      startElem
    ).getBoundingClientRect();
    const endElementCoords = ReactDOM.findDOMNode(
      startElem
    ).getBoundingClientRect();
  ...
  dfs = (root) => {
    ...
    setTimeout(
      ...
      (this.state.nodeCount - this.count + 1) * 500
    );
    this.count--;
    this.dfs(root.right);
  };
  ...
  handleLCAClick = (node) => {
    let lca = this.getLowestCommonAncestor(
      this.state.root,
    ...
    ref.current.style.backgroundColor = this.state.styles[
      'lca'
    ].backgroundColor;
  ...
  handleNodeClick = (node) => {
    ...
    if (selectedNodes.length >= 2) {
      ...
      poppedRef.current.style.backgroundColor = this.state.styles[
        'default'
      ].backgroundColor;
      poppedRef.current.style.borderColor = this.state.styles[
        'default'
      ].borderColor;
  ...
  render = () => {
    ...
    let selectedIds = new Set(
      this.state.selectedNodes.map((x, xi) => {
    ...
    while (cur != null || stack.length > 0) {
      ...
      if (parentId != null) {
        ...
        paths.push(
          <path
            key={pathId}
            ref={this.state.pathRefs[pathId]}
            id={pathId}
            d="M0 0"
            stroke="#014048"
            fill="none"
      ...
      <div>
        ...
        <Grid container style={{ padding: '10px' }}>
          <Grid item xs={3}>
            ...
            >
              <Button
                onClick={this.onDFSClick}
                disabled={this.state.root == null}
              >
                DFS
              </Button>
              <Button
                onClick={this.onBFSClick}
                disabled={this.state.root == null}
              >
                BFS
              </Button>
              <Button
                onClick={this.onBoundaryClick}
                disabled={this.state.root == null}
              >
                Boundary
              </Button>
              <Button
                onClick={this.onSerialize}
                disabled={this.state.root == null}
              >
                Serialize
              </Button>
          ...
          <Grid item xs={3}>
            <div component="form" style={styles.root}>
              ...
              <IconButton
                ...
                aria-label="search"
                onClick={this.handlePathSumSearch}
              >
                <SearchIcon />
              </IconButton>
            </div>
